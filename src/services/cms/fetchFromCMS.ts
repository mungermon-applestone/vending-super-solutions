...
async function fetchProductTypes<T>(params: Record<string, any> = {}): Promise<T[]> {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      console.warn(`[CMS Service] No product types found${params.slug ? ` for slug ${params.slug}` : ''}`);
      return [] as T[];
    }

    console.log(`[CMS Service] Found ${data.length} product types${params.slug ? ` for slug ${params.slug}` : ''}`);
